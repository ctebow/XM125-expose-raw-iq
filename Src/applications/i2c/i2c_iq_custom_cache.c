...
//
// Cached variant of i2c_iq_custom:
// - Keeps the external style-A protocol identical: command op=6, index in low 16 bits.
// - Captures a full IQ frame only when cache is invalid (iq_valid == false).
// - Subsequent IQ commands only update iq_current_index and return cached I/Q via existing registers.
//
// IMPORTANT:
// - This file is intended to REPLACE i2c_iq_custom.c in the build (do not compile both).
// - No extra protocol registers, no timestamps, no printf/logging to keep FLASH impact minimal.
//
...
void command_handler(uint32_t command)
{
	...
	{
		// Cached IQ path:
		// - If cache invalid: capture once to fill buffers and set iq_valid/iq_num_points.
		// - Always: update iq_current_index (clamped) and return.

		acc_integration_critical_section_enter();
		bool     cached_valid = iq_valid;
		uint16_t cached_len   = iq_num_points;
		acc_integration_critical_section_exit();

		if (!cached_valid)
		{
			// Clear state before capture.
			acc_integration_critical_section_enter();
			iq_valid         = false;
			iq_num_points    = 0U;
			iq_current_index = 0U;
			...
			if (my_capture_iq_frame(i2c_distance_detector_core_get_resources(),
			                        iq_real_buffer,
			                        iq_imag_buffer,
			                        &num_points) &&
			    num_points > 0U)
			{
				// Clamp requested index to captured length.
		...
		// Cache is valid: just update index (clamped to cached_len).
		uint16_t requested_index = index;
		if (cached_len == 0U)
		{
			// Shouldn't happen if cached_valid is true, but keep behavior safe.
			return;
		}
		if (requested_index >= cached_len)
		{
			requested_index = (uint16_t)(cached_len - 1U);
		}

		acc_integration_critical_section_enter();
		iq_current_index = requested_index;
		// iq_valid and iq_num_points remain unchanged.
		acc_integration_critical_section_exit();
		return;
	}

	// Any command that changes detector state should invalidate cached IQ.
